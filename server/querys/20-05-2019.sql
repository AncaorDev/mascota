...
BEGIN
	...
	-- Datos por recomendación
		WITH scraper AS (
			...
			SELECT *
	          FROM filtro_1
             WHERE TRUE = ((SELECT __func_02_exist_value_in_tags(LOWER(nombre), (__tag_age || __tag_feeding || __tag_race || __tag_size))) OR (SELECT __func_02_exist_value_in_tags(LOWER(descripcion), (__tag_age || __tag_feeding || __tag_race || __tag_size))))
